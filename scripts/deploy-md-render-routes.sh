#!/usr/bin/env bash
# Attach brmste-md-render Worker to /docs on primary BRMSTE zones.
# Requires CF_API_TOKEN (operator-managed in GitHub Actions — never in chat).
set -euo pipefail

: "${CF_API_TOKEN:?CF_API_TOKEN must be set}"

CF_ACCOUNT_ID="${CF_ACCOUNT_ID:-7ea6547b1d6eb1cbd6d0ac5cf960ce2a}"
WORKER_NAME="${WORKER_NAME:-brmste-md-render}"
