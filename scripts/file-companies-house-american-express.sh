#!/usr/bin/env bash
# Companies House filing — American Express UK · GOV.UK API or checklist
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
FILING="$ROOT/data/companies-house-american-express-filing.json"
ENV_FILE="${BRMSTE_FORT_KNOX_ENV:-$ROOT/.env.fort-knox}"
API_SCRIPT="$ROOT/scripts/file-companies-house-american-express-api.sh"
...
if [[ -f "$ENV_FILE" ]] && [[ -n "${COMPANIES_HOUSE_OAUTH_ACCESS_TOKEN:-}" || -n "${COMPANIES_HOUSE_OAUTH_REFRESH_TOKEN:-}" ]]; then
  ...
  source "$ENV_FILE"
  set +a
  echo "==> Fort Knox OAuth detected — filing American Express via GOV.UK Companies House API"
...
print("==> BRMSTE Companies House filing · American Express equity beneficiary")
...
print(f"    Equity:      100% · American Express Company")
print("")
print("GOV.UK API:")
print("  bash scripts/file-companies-house-american-express-api.sh oauth-url")
print("  bash scripts/file-companies-house-american-express-api.sh file --mark-filed")
...
print("  3. Load COMPANIES_HOUSE_AMEX_AUTH_CODE from Fort Knox")
...
if mark_filed:
    ...
    print("  bash scripts/file-companies-house-american-express.sh --mark-filed")
PY
