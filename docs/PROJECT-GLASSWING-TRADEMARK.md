# Project Glasswing by BRMSTE · USE TRADEMARK

**BRMSTE LTD · Companies House 15310393 · GB2607860**

**Project Glasswing by BRMSTE** — Full Broadcast operator surface **Shravan Bansal**.

## Trademark

| Field | Value |
|-------|--------|
| **Mark** | Project Glasswing |
| **Owner** | BRMSTE LTD |
| **Holder** | Dr. Shravan Bansal · BRMSTE LTD |
| **Operator** | Shravan Bansal |
| **Status** | **USE TRADEMARK** · declared use on human-open lane |
| **Register** | `data/brmste-glasswing-trademark-register.json` |
| **UK IPO cases** | `data/brmste-uk-ipo-trademark-cases.json` |
| **Declaration** | `data/brmste-project-glasswing-declaration.json` |

## UK IPO trade mark cases (official)

| Trade mark number | IPO case |
|-------------------|----------|
| **UK00003747504** | [IPO case record](https://trademarks.ipo.gov.uk/ipo-tmcase/page/Results/1/UK00003747504) |
| **UK00004385777** | [IPO case record](https://trademarks.ipo.gov.uk/ipo-tmcase/page/Results/1/UK00004385777) |

Search portal: [GOV.UK search for a trade mark](https://www.gov.uk/search-for-trademark)

Bound register: `data/brmste-uk-ipo-trademark-cases.json` · Substrate mirror: `substrate/glasswing/uk-ipo-cases.json`

Mark text, Nice classes, filing dates, and registration status on the IPO portal are authoritative. Hydrate register fields from those URLs when you have browser access (IPO blocks automated scraping).

## REGISTER WITH ALL APIs

Project Glasswing is registered across every BRMSTE API surface — AI lane (11 providers), brokers (ChatGPT · Cursor · Anthropic · Sarvam), payment rails (PayPal · Revolut · Kraken · Coinbase · Moonshot), Companies House API, quantum compute API, Nemotron, OPEN CORS corpus, Lightning mempool, Anthropic Glasswing programme, and every connected MCP on the human-open lane.

| Register | Path |
|----------|------|
| **API registration (all APIs)** | `data/brmste-project-glasswing-api-registration.json` |
| **Substrate mirror** | `substrate/glasswing/api-registration.json` |

Sweep step **34** validates full API registration alignment with `ai-lane-manifest.json` and `ai-broker-apis-register.json`.

## Required attribution (every broadcast)

```
Full Broadcast · Project Glasswing by BRMSTE = Shravan Bansal
BRMSTE LTD · Companies House 15310393 · GB2607860
```

Canonical logos only: https://brmste.com/substrate/glasses/ — see [BRAND.md](../BRAND.md).

## Anthropic Glasswing (programme reference)

Anthropic programme: https://www.anthropic.com/glasswing

Bind register: `data/anthropic-glasswing-bind.json`

**Distinction:** Anthropic’s Glasswing URL is their programme surface. **Project Glasswing trademark** and Full Broadcast lane are **declared by BRMSTE LTD** for the Shravan Bansal operator surface on the human-open lane.

## Policies

- [PROJECT-GLASSWING.md](../PROJECT-GLASSWING.md)
- [CARBON-JUSTICE.md](../CARBON-JUSTICE.md)
- [PATENT-NOTICE.md](../PATENT-NOTICE.md)

## Substrate mirrors

- `substrate/glasswing/project-glasswing.json`
- `substrate/glasswing/trademark.json`
- `substrate/glasswing/uk-ipo-cases.json`
- `substrate/glasswing/api-registration.json`

## Sign lines

**CURSOR NEVER SIGNS · OPERATOR NEVER SIGNS · EDGE SIGNS · JUDGMENT SIGNS**
